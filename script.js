...
function renderSchedule(container, dayData, viewingToday, day) {
  ...
  dayData.forEach((item, index) => {
    ...
    const editIcon = `<div class="edit-hint">✏️</div>`;
    ...
    for (const prof in profiles) {
      for (const day in profiles[prof]) {
        ...
        arr.forEach((item, idx) => {
          ...
          if (!def) return;
          const diff = {};
          ["subject","time","room","teacher","tag"].forEach(k => {
...
// --- UPDATED: FEATURE 1 — all cards editable, FEATURE 3 — pre-fill tag ---
function openModal(dayKey, itemIndex) {
  const item = profiles[currentProfile][dayKey][itemIndex];
  if (!item) return;

  // REMOVED: break guard — all cards are now editable

  modalDayKey    = dayKey;
  modalItemIndex = itemIndex;
  isEditing      = false;

  // RULE 1: Normalize break card data before populating modal
  // This catches legacy cards tagged as break but with non-"Break" subjects
  const tag = getTag(item);
  if (tag === "break") {
    item.subject = "Break";
    item.room    = "";
...
function setReadonly(val) {
  ...
  // Also lock/unlock the tag select
  const tagSel = document.getElementById("editTag");
  if (tagSel) tagSel.disabled = val;
}

/* --- NEW: FEATURE 5/6 — show/hide room+teacher based on break state --- */
function updateModalBreakState(isBreak, readonly) {
  ...
  if (isBreak) {
    ...
    // Visual cue: subject is locked on break cards
...
/* --- UPDATED: Live tag change — enforces break data rules immediately --- */
function onTagChange() {
  if (!isEditing) return;
  const isBreak = document.getElementById("editTag").value === "break";
  updateModalBreakState(isBreak, false);
  if (isBreak) {
    // RULE 1: Break must show "Break" as subject and clear room/teacher
    ...
    // RULE 2: Disable subject for break cards
    document.getElementById("edit-subject").setAttribute("readonly", true);
  } else {
    // RULE 3: Switching away from break — re-enable subject
    // If subject is still "Break" (stale), reset it so user enters the real name
    const subjectEl = document.getElementById("edit-subject");
    subjectEl.removeAttribute("readonly");
    if (subjectEl.value === "Break") {
      subjectEl.value = "";
    }
    subjectEl.focus();
  }
...
// --- UPDATED: store tag in originalValues; respect break state on edit ---
function toggleEdit() {
  isEditing = true;
  originalValues = {
  ...
  const isBreak = document.getElementById("editTag").value === "break";
  updateModalBreakState(isBreak, false);
  // RULE 2: Subject must remain locked for break cards
  if (isBreak) {
    document.getElementById("edit-subject").setAttribute("readonly", true);
  }
  ...
  // Focus time field for break cards (subject is locked), else subject
  document.getElementById(isBreak ? "edit-time" : "edit-subject").focus();
}
...
/* UPDATED: FEATURE 4/5/7/9 — tag save, break data cleanup, re-sort */
function saveEdit() {
  const hint    = document.getElementById("modalHint");
  const tag     = document.getElementById("editTag").value;
  const isBreak = (tag === "break");

  // RULE 5/7: For break cards, always enforce canonical values
  ...
  // Validation — skip subject check for break (it's always "Break")
  if (!isBreak && !subject) {
  ...
  // RULE 7: Apply data integrity — break cards must NEVER carry stale subject/room/teacher
